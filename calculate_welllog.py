...
def plot_relative_permeability(df, depth_start, depth_end, n, m):
    # 选择特定深度区间的数据
    max_depth = df['DEPT'].max()
    if depth_end > max_depth:
        depth_end = depth_start - 20
    dataindex = df[(df['DEPT'] >= depth_start) & (df['DEPT'] <= depth_end)]

    # 如果没有找到对应深度的数据，打印错误信息并返回
    if dataindex.empty:
        print(f"No data found at depth range {depth_start} ")
        print(f"All data depth range: {df['DEPT'].min()} to {df['DEPT'].max()}")
        return
    # 计算相对渗透率
...
# df = pd.read_excel('D:/File/24石工大赛/project/py/pyProject/calculate/007facies.xlsx')
# df, depth = calculate_(df)
# # 使用函数生成图像
# a = 2641.11
# plot_relative_permeability(df,a, a + 20, n=1.5, m=2.2)  # 生成深度为1000到2000的相对渗透率曲线
# plt1(df)
